...
def adagrad(cost, params, lr=np.float32(0.01), eps=np.float32(1e-4)):
    g_params = tf.gradients(cost, params)
    updates = []
    for param, g_param in zip(params, g_params):
...
def adadelta(cost, params, gamma=np.float32(0.95), eps=np.float32(1e-3)):
    g_params = tf.gradients(cost, params)
    updates = []
    for param, g_param in zip(params, g_params):
        ...
        updates.append(ms_g.assign(gamma*ms_g + (np.float32(1.0) - gamma)*g_param**2))
        with tf.control_dependencies(updates):
            updates.append(d_param.assign(-tf.sqrt((ms_d + eps)/(ms_g + eps))*g_param))
        with tf.control_dependencies(updates):
            updates.append(param.assign(param + d_param))
        with tf.control_dependencies(updates):
            updates.append(ms_d.assign(gamma*ms_d + (np.float32(1.0) - gamma)*d_param**2))
    return updates
...
def smorms3(cost, params, lr=np.float32(0.001), eps=np.float32(1e-8)):
    g_params = tf.gradients(cost, params)
    updates = []
    for param, g_param in zip(params, g_params):
